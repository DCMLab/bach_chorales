...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/bach_chorales.git
...
* `Johann Sebastian Bach – The Chorales <https://github.com/DCMLab/bach_chorales/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by five files with identical names, each in its own folder. For example, the first chorale has the following files:

-  ``MS3/001 Aus meines Herzens Grunde.mscx``: Uncompressed MuseScore 3.6.2 file including the music and annotation labels.
-  ``notes/001 Aus meines Herzens Grunde.notes.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/001 Aus meines Herzens Grunde.measures.tsv``: A table with relevant information about the measures in the score.
-  ``chords/001 Aus meines Herzens Grunde.chords.tsv``: A table containing layer-wise unique onset positions with the musical markup (such as dynamics, articulation, lyrics, figured bass, etc.).
-  ``harmonies/001 Aus meines Herzens Grunde.harmonies.tsv``: A table of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/001 Aus meines Herzens Grunde.tsv')
   notes = ms3.load_tsv('notes/001 Aus meines Herzens Grunde.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/bach_chorales/issues>`__ and feel free to fork and submit pull requests.
